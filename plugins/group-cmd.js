...
async (conn, mek, m, { from, sender, isGroup, reply, quoted, participants }) => {
    try {
        ...
        try {
            ...
            ppUrl = 'https://files.catbox.moe/kh4wjs.png'; // Default image if no profile picture
        }
...
cmd({
    ...
    try {
        ...
        reply(`Approved the following +91 users:\n${userJids.join("\n")}`);
...
if (!isAdmins) return reply('You Are Not A Admin')	
                ...
                setTimeout(() => {
                    ...
                    const close = `*CLOSE TIME* GROUP CLOSED BY Neon-AI AT APPROVED ADMIN\nNOW ONLY ADMIN CAN SEND MESSAGES 🔐`
...
if (!isAdmins) return reply('You Are Not A Admin')
const G_MUTE = `*GROUP IS CLOSED BY Neon-AI BOT OWNER*`;
...
if (!isBotAdmins) return reply('You Not A Bot Owner')
if (!isAdmins) return reply('You Are Not A Admin')
const G_UNMUTE = `*GROUP IS OPEN BY Neon-AI BOT OWNER*`;
...
await conn.sendMessage(from, { react: { text: `✅`, key: mek.key }}) 
reply('*GROUP ADMIN PROMOTE BY Neon-Coder BOT OWNER*')
} catch (e) {
reply('*ERROR*')
console.log(e)
}
...
await conn.sendMessage(from, { react: { text: `✅`, key: mek.key }})
reply('*GROUP ADMIN DEMOTE BY Neon-Coder BOT OWNER*') 
} catch (e) {
reply('*ERROR*')
console.log(e)
}
