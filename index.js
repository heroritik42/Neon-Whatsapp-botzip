...
// Owner numbers
const ownerNumber = ["916205232179"]; // Add your number here with country code
...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/arslan.html')) {
    console.log("✅ Neon-AI.html found in lib folder");
  } else {
    console.log("⚠️ Neon-AI.html not found in lib folder");
  }
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'NEON-CODER'}</p>
        </div>
      </body>
      </html>
    `);
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://files.catbox.moe/kh4wjs.png'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 Neon-AI Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ Neon-AI Bot connected to WhatsApp!");
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '916205232179@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://files.catbox.moe/kh4wjs.png');
